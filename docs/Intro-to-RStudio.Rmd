...
* Make a `Pace-R` directory / folder
* Make a `data` directory / folder within `Pace-R`
* Make a `scripts` directory / folder within `Pace-R`
* In RStudio, set working directory to your new ``Pace-R`` directory
...
```{r, eval=FALSE}
setwd("/Users/maiellolammens/Pace-R/")
```

### Making an R Project

Let's make a new R Project associated with your Pace-R directory. 
...
Use the graphical user interface (GUI) to navigate to the Pace-R directory, then select "Create Project".
...
```{r soil_data}
soil_data <- read.csv("https://mlammens.github.io/Pace-R-Wallace/docs/soil_nutrient_data.csv")
```
